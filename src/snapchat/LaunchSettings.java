...
public class LaunchSettings extends UiAutomatorTestCase {
	...
	protected void performLogin(String username, String password) {
		...
		if (Utils.hasObject(ID_LOGIN_BUTTON)) {
			...
			/* Click login button */
			assertTrue("Unable to click on the login button", Utils.clickAndWaitForNewWindow(ID_LOGIN_BUTTON));

			/* Insert username, password and click login button */
			assertTrue("Email field not available", Utils.setText(ID_LOGIN_EMAIL, username));
			assertTrue("Password field not available", Utils.setText(ID_LOGIN_PASSWD, password));
			assertTrue("Login button not available", Utils.clickAndWaitForNewWindow(ID_LOGIN_LOGIN));
		}
	}
	...
	protected void snapAPicture(String message, String[] recipients) {
		...
		assertTrue("Unable to set picture message", Utils.setText(ID_SNAP_MESSAGE, message));
		getUiDevice().pressBack();
		sleep(1000);*/
		
		assertTrue("Send button not available", Utils.clickAndWaitForNewWindow(ID_SNAP_SEND));
		
		List<UiObject> available = Utils.getElems(ID_LIST_RECIPIENT, ID_LIST_NAME);
		assertTrue("Unable to retrieve the recipients", !available.isEmpty());
		
		List<String> checked = new ArrayList<String>();
		for (UiObject dest : available) {
			for (String recipient : recipients){
				if(Utils.hasText(dest,recipient) && !checked.contains(recipient)){
					checked.add(recipient);
					assertTrue("Unable to select recipient", Utils.hasTextAndClick(dest, recipient));
				}
			}
		}
		assertTrue("Send button in recipient list not available", Utils.click(ID_LIST_SEND));
   ...
   public void testDemo() throws UiObjectNotFoundException{
      assertTrue("OOOOOpps",Utils.openApp(this, "Snapchat", "com.snapchat.android"));
